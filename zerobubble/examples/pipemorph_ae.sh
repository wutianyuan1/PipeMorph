...
cd /$USR_HOME/workspace/PipeMorph-AE/zerobubble
source ~/.bashrc
echo `pwd`
DIR=`pwd`
...
DATASET="/$USR_HOME/workspace/PipeMorph-AE/zerobubble/zb_sample_dataset/dataset/c4_text_document"
...
if [ ! -e "$DATASET"".idx" ]; then
  ...
  tar -xvf zb_sample_dataset.tar.gz -C /$USR_HOME/workspace/PipeMorph-AE/zerobubble/
fi
...
options=" \
  ...
  --tokenizer-model /$USR_HOME/workspace/PipeMorph-AE/zerobubble/zb_sample_dataset/tokenizers/tokenizer.model \
  --split 98,2,0 \
  --clip-grad 8.0 \
  --weight-decay 0.1 \
  --adam-beta1 0.9 \
