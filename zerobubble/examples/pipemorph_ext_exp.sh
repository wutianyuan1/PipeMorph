...
cd /$USR_HOME/workspace/PipeMorph-Ext-Exp/zerobubble
source ~/.bashrc
echo `pwd`
DIR=`pwd`
...
DATASET="/$USR_HOME/workspace/PipeMorph-Ext-Exp/zerobubble/zb_sample_dataset/dataset/c4_text_document"
...
if [ ! -e "$DATASET"".idx" ]; then
  ...
  tar -xvf zb_sample_dataset.tar.gz -C /$USR_HOME/workspace/PipeMorph-Ext-Exp/zerobubble/
fi
...
options=" \
  ...
  --tokenizer-model /$USR_HOME/workspace/PipeMorph-Ext-Exp/zerobubble/zb_sample_dataset/tokenizers/tokenizer.model \
  --split 98,2,0 \
  --clip-grad 8.0 \
  --weight-decay 0.1 \
  --adam-beta1 0.9 \
...
if [[ $RANK -eq 0 ]]; then
  ...
  redis-server --bind $MASTER_ADDR --save "" --port 9969 --appendonly no --protected-mode no &
  REDIS_PID=$!
fi
