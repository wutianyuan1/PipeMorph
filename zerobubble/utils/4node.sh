...
for i in "${!nodes[@]}"; do
    ...
    ssh -i ~/workspace/aliyun.pem $node "/root/miniconda3/bin/torchrun --nnodes 4 --nproc-per-node 1 --node-rank ${i} --master-addr 172.24.82.221 --master-port 9969 /root/workspace/PipeMorph/zerobubble/megatron/core/pipeline_parallel/new_delegate.py" &
done
