...
for method in METHODS:
    ...
    for i, model in enumerate(MODELS):
        if method != 'ALL-CPU':
            dir = os.path.join(PATH, method, '0_1', '0.06')  # choose the beginning iters of 0_1/0.06
        else:
            dir = os.path.join(ALL_CPU_PATH, method)

        try:
                iter_times = parse_iter_times(dir, method, (2, 7))
                iter_times /= 1000
...
plt.savefig(f'{ALL_CPU_PATH}/fig20.pdf', bbox_inches='tight', bbox_extra_artists=(legend,))
plt.savefig(f'{ALL_CPU_PATH}/fig20.png', bbox_inches='tight', bbox_extra_artists=(legend,))
