...
private:
  ...
  std::vector<Queue *> fwd_queues_, bi_queues_, bw_queues_;
  int pipeline_depth_;
  int num_mini_;
  int rank_;
};
...
void print_task(std::vector<schedule_task>* all_tasks, int num_stages) {
  char taskmap[5] = "FRbB"; 
  int maxt = 0;
  char** pp_schedule = new char*[num_stages];
  for (int j = 0; j < num_stages; j++) {
    pp_schedule[j] = new char[100];
    for (int i = 0; i < 100; i++) pp_schedule[j][i] = 'Z';
    auto tasks = all_tasks[j];
    for (int i = 0; i < tasks.size(); ++i) {
      pp_schedule[j][tasks[i].time] = taskmap[tasks[i].task - '0'];
      maxt = (maxt > tasks[i].time) ? maxt : tasks[i].time;
    }
    // printf("%c,%d;\n", taskmap[tasks[i].task - '0'], tasks[i].index);
  }

  for (int j = 0; j < num_stages; j++) {
    printf("Stage%d: [", j);
    for (int i = 0; i < maxt; i++) printf("%c ", pp_schedule[j][i]);
    printf("]\n");
  }

  for (int j = 0; j < num_stages; j++) {
    delete[] pp_schedule[j];
  }
  delete[] pp_schedule;
}
...
void GenSchedule::Generate()
{
  ...
  for (time = 0; time < 20000; ++time)
  {
    ...
    for (int i = 0; i < pipeline_depth_; ++i)
    {
      ...
      if (identifier != '3')
      {
        schedule_task a = {mini, identifier, time};
        sched[i].push_back(a);
      }
    }
    // printf("\n");
