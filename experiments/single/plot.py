...
for sid, slowlink in enumerate(['First', 'Last']):
    for figid, delay in enumerate(DELAYS):
        ...
        plt.xlabel(f"{'60 ms' if delay == '0.06' else '30 ms'} delay between {slowlink.lower()} two stages", fontsize=14)
        if figid == 0:
...
plt.savefig(f'{PATH}/{slowlink}_merge.pdf', bbox_inches='tight', bbox_extra_artists=(legend,))
plt.savefig(f'{PATH}/{slowlink}_merge.png', bbox_inches='tight', bbox_extra_artists=(legend,))
plt.close()
...
for link in ['First', 'Last']:
    ppm_cpu = opt_ratio['ZB-CPU'][link]['0.06']
    print('1 - PipeMorph / PipeMorph-CPU', ppm_cpu)
    print('1 - PipeMorph / PipeMorph-CPU', 1 - t['ZB-CPU-ReSchedule'][link]['0.06'] / t['ZB-CPU'][link]['0.06'])
    print(f'avg {sum(ppm_cpu) / len(ppm_cpu):.3f}')
    print(f'max {max(ppm_cpu):.3f}')
    print()
# delay
for method in METHODS:
    vals1 = np.concatenate([t[method][slowlink]['0.03'] for slowlink in ['First', 'Last']], axis=0)
    vals2 = np.concatenate([t[method][slowlink]['0.06'] for slowlink in ['First', 'Last']], axis=0)
    print(vals1)
    print(vals2)
