...
parser.add_argument("--num_change_timings", type=int, default=20)
args = parser.parse_args()

LINKS = [f'{i}_{i + 1}' for i in range(args.num_stages - 1)]
# Distribution parameters
MIN_DURATION, MAX_DURATION = 500, 1000
DELAY_MEAN, DELAY_STD = 30, 10
...
for i in range(args.num_change_timings):
...
# plt.xlabel('Wallclock Time (s)')
# plt.ylabel('Delay (ms)')
plt.savefig("/root/workspace/PipeMorph-Ext-Exp/ext_exp/dynamic_stragglers/trace.png")

with open(f"/root/workspace/PipeMorph-Ext-Exp/zerobubble/megatron/core/failslow_injection/slowlinks.trace", 'w') as f:
    f.writelines([f"{timing / 1000};{','.join(link2delay.keys())};{','.join([str(t / 1000) for t in link2delay.values()])}\n" for timing, link2delay in trace])